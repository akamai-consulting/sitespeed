...
# This script has been modified to work on Linode with multiple hosts
  ...
  do
    ...
    case $1 in
        tld) docker run \
             ...
             sitespeedio/sitespeed.io:24.1.0 \
             --config config.json \
             $SitespeedDirs \
...
echo "sitespeed_usage.$1.$logname.duration $runtime `date +%s`" | nc 198.74.57.70 2003
echo "sitespeed_usage.$1.$logname.disk `ssh 198.74.57.70 du -s graphite-storage/whisper/sitespeed_io/$graphdir/$2 | awk '{print $1}'` `date +%s`" | nc 198.74.57.70 2003
echo "sitespeed_usage.$1.$logname.images `du -s $(pwd)/portal/images/$2 | awk '{print $1}'` `date +%s`" | nc 198.74.57.70 2003
