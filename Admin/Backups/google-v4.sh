...
# Set variable Sitespeed version
SitespeedVer="sitespeedio/sitespeed.io:25.11.0-plus1"
...
DockerLAN="--rm --name $2-`date +%s`"
DockerMobile="--rm --name $2-`date +%s` --cap-add=NET_ADMIN"

# Set Sitespeed directives
ChromeLAN="--browsertime.connectivity.alias Desktop"
ChromeLTE="-c custom --browsertime.connectivity.alias Mobile --downstreamKbps 12000 --upstreamKbps 12000 --latency 35 --connectivity.engine throttle --chrome.CPUThrottlingRate 4 --mobile"
...
# Quick check to ensure that network throttling will work          
...
docker run \
 ...
 $ChromeLTE \
 --name "$2 over LTE with CrUX & GPSI Data" \
 --slug $3 \
...
docker run \
 ...
 --crux.collect "ALL" \
 $ChromeLAN \
 --name "$2 over LAN with CrUX & GPSI Data" \
 --slug $3 \
 --graphite.namespace sitespeed_io.$graphdir.$2 \
 $url
