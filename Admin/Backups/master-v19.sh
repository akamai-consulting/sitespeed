...
DockerDesktop="--rm --name $2"
DockerMobile="--rm --name $2 --cap-add=NET_ADMIN"

# Connectivity Sitespeed directives
ChromeNative="--browsertime.connectivity.alias LAN"
...
echo "sitespeed_usage.$1.$logname.images `du -s $(pwd)/portal/images/$2 | awk '{print $1}'` `date +%s`" | nc -N 10.128.0.11 2003
