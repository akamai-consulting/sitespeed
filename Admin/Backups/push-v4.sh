...
function summary {
  if [ "$1" == "index" ]
    ...
    elif [ $successCnt -eq $(echo $Regions | wc -w) ]
      then
        echo "$successCnt Linodes updated"
    elif [ $failureCnt -gt 0 ]
      then
        echo "$successCnt Linodes updated"
...
case $1 in
            ...
            for region in $Regions
              do
                ...
                update ) echo -n "Starting "$region" ... "
                         ...
                         scp Sitespeed/mycron.TMP greg@"$region":mycron &> /dev/null
   ...
   docker ) if [ $# -ne 2 ]
    ...
    index ) echo -n "Starting copy to US-East ... "
            ...
            for region in $Regions
              do
                case $2 in
                  check ) tldErrorCnt=0
                          compErrorCnt=0
                          ssh greg@"$region" ls ./logs/tld.msg.log &> /dev/null
                          if [ $? -eq 0 ]
                            then
                              tldErrorCnt=$(ssh greg@"$region" grep -i -E 'error\|unhandled\|fail\|caught\|invalid\|undefined\|timeout' ./logs/tld.msg.log | wc -l)
                          fi
                          ssh greg@"$region" ls ./logs/comp.msg.log &> /dev/null
                          if [ $? -eq 0 ]
                            then
                              compErrorCnt=$(ssh greg@"$region" grep -i -E 'error\|unhandled\|fail\|caught\|invalid\|undefined\|timeout' ./logs/comp.msg.log | wc -l)
                          fi
                 ...
                 delete ) echo -n "Starting "$region" ... "
                          ...
                          if [ $? -eq 0 ]
                            then
                              ssh greg@"$region" rm ./logs/tld.msg.log
                          fi
                          ssh greg@"$region" ls ./logs/comp.msg.log &> /dev/null
                          if [ $? -eq 0 ]
                            then
                              ssh greg@"$region" rm ./logs/comp.msg.log
                          fi
                          chkresult
                esac         
              done
   ...
   update ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                ssh -q greg@"$region" sudo yum -y update &> /dev/null
                chkresult
              done
              summary
              exit 0
