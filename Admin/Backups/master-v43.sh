...
# Set variables
SitespeedVer="sitespeedio/sitespeed.io:26.0.1"
...
for (( index=1; index < 3 ; index+=1 ))
  do
    ...
    docker run \
     ...
     -n $ITR \
     --config config.json \
     --resultBaseURL "https://$3.sitespeed.akamai.com/$1/sitespeed-result" \
     $SitespeedDirs \
     --name "$RptName" \
     --slug $3 \
...
for domain in $domainList
  do
    mv -f $(pwd)/$1/sitespeed-result/$3/"$domain"* $(pwd)/portal/images/$2/ &> /dev/null
  done
...
echo "sitespeed_log.$3.duration $runtime `date +%s`" | nc graphite.$Domain 2003
echo "sitespeed_log.$3.errors $(grep -i error ~/logs/$1.$2.msg.log | wc -l) `date +%s`" | nc graphite.$Domain 2003
