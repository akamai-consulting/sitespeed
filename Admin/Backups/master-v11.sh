...
case $1 in
    tld) docker run \
         ...
         --visualElements \
         --cpu \
         --resultBaseURL https://www.hilltoptech.net/tldroot \
  ...
  comp)  docker run \
         ...
         --visualElements \
         --cpu \
         --resultBaseURL https://www.hilltoptech.net/comproot \
         --copyLatestFilesToBase true \
         --screenshot.type png \
         --thirdParty.cpu \
         --chrome.collectLongTasks \
...
echo "sitespeed_usage.$1.$logname.disk `ssh 10.128.0.11 du -s /home/greg/graphite-storage/whisper/sitespeed_io/$graphdir/$2 | awk '{print $1}'` `date +%s`" | nc -N 10.128.0.11 2003
