...
case $1 in
    tld) docker run \
         ...
         --visualElements \
         --cpu \
         --resultBaseURL http://www.hilltoptech.net \
...
echo "sitespeed_usage.$1.$logname.duration $runtime `date +%s`" | ncat 10.128.0.11 2003
echo "sitespeed_usage.$1.$logname.disk `ssh 10.128.0.6 du -s /home/greg/graphite-storage/whisper/sitespeed_io/$graphdir/$2 | awk '{print $1}'` `date +%s`" | ncat 10.128.0.11 2003

# Set the symlink for nginx root directive to point to the latest index.html
ln -nsf $(find $(pwd)/$1/sitespeed-result/ -maxdepth 3 -name index.html | xargs ls -Art | tail -n 1 | xargs dirname) /home/greg/portal/$webroot
