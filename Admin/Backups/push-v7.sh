...
if [[ "$1" == "--help" || "$1" == "-h" || "$1" == "/?" || $# -eq 0 ]]
    then
        ...
        echo -e "\t${Green}master${NoColor}\tCopies master.sh to $HOME across all Linodes\n"
              ...
              do
                case $2 in
                  check ) ssh greg@"$region" ls ./logs/tld.*.msg.log &> /dev/null
                          ...
                          ;;
                 delete ) echo -n "Starting "$region" ... "
                          ssh greg@"$region" ls ./logs/tld.*.*.log &> /dev/null
                          if [ $? -eq 0 ]
                            then
                              ssh greg@"$region" rm ./logs/tld.*.*.log
                          fi
                          ssh greg@"$region" ls ./logs/comp.*.*.log &> /dev/null
                          if [ $? -eq 0 ]
                            then
                              ssh greg@"$region" rm ./logs/comp.*.*.log
                          fi
                          chkresult
                          ;;
                esac         
