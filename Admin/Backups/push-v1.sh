...
# Function that checks the results of each action
function chkresult {
  if [ $? -eq 0 ]
    then
      let "successCnt++"
...
function summary {
  echo -e "Total updated Linodes = $successCnt"
  if [ $failureCnt -ne 0 ]
    then
      echo -e "Failed Linodes = $failure\n"
  fi
}
...
case $1 in
      all ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                scp Sitespeed/*.sh greg@"$region":/home/greg
                chkresult
              done
              summary
              exit 0
   ...
   config ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                scp Sitespeed/config.json greg@"$region":/home/greg/tld
                scp Sitespeed/config.json greg@"$region":/home/greg/comp
                chkresult
              done
              summary
              exit 0
              ...
              do
                case $2 in
                  list ) ssh greg@us-east crontab -l
                         exit 0
                         ;;
                update ) echo -e "Starting "$region" ..."
                         sed 's/XXX/\'$region'/' Sitespeed/mycron > Sitespeed/mycron.TMP
                         scp Sitespeed/mycron.TMP greg@"$region":/home/greg/mycron
                         ssh greg@"$region" crontab mycron
                         chkresult
                         ssh greg@"$region" rm /home/greg/mycron
                         rm Sitespeed/mycron.TMP
                         ;;
                delete ) echo -e "Starting "$region" ..."
                         ssh greg@"$region" crontab -r
                         chkresult
                         ;;
                esac         
              done
   ...
   docker )   if [ $# -ne 2 ]
              ...
              for region in $Regions
                do
                  echo -e "Starting "$region" ..."
                  ssh greg@"$region" docker system prune --all --volumes -f
                  ssh greg@"$region" docker pull sitespeedio/sitespeed.io:$2
                  chkresult
                done
                summary
                exit 0
   ...
   master ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                scp Sitespeed/master.sh greg@"$region":/home/greg
                chkresult
              done
              summary
              exit 0
    ...
    nginx ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                ssh greg@"$region" sudo /home/greg/nginx.sh portal
                ssh greg@"$region" sudo /home/greg/nginx.sh tld
                ssh greg@"$region" sudo /home/greg/nginx.sh comp
                chkresult
              done
              summary
              exit 0
    ...
    reset ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                ssh greg@"$region" /home/greg/reset.sh
                chkresult
              done
              summary
              exit 0
     ...
     seed ) if [ $# -ne 3 ]
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                scp Seeds/$3.txt greg@"$region":/home/greg/$2
                chkresult
              done
              summary
              exit 0
   ...
   update ) successCnt=0
            ...
            for region in $Regions
              do
                echo -e "Starting "$region" ..."
                ssh greg@"$region" sudo yum -y update
                chkresult
              done
              summary
              exit 0
