...
Green='\033[0;32m'
NoColor='\033[0m'
Options="all update docker cwv index master config seed reset nginx cron log sro"
...
if [[ "$1" == "--help" || "$1" == "-h" || "$1" == "/?" || $# -eq 0 ]]; then
   ...
   echo -e "\t${Green}cwv${NoColor}\tCopies CWV files to US-East. Requires:\n"
   echo -e "\t\t${Green}arg2${NoColor} = script|seed"
   echo -e "\t\t${Green}arg3${NoColor} = Name of URL seed file\n"
   echo -e "\t${Green}index${NoColor}\tCopies index.html across all Linodes\n"
   ...
   echo -e "\t\t${Green}arg3${NoColor} = Name of URL seed file\n"
   echo -e "\t${Green}sro${NoColor}\tCopies Sure Route Object across all Linodes\n"     
...
case $1 in
      all ) for region in $Regions
              do
                ...
                scp Sitespeed/*.sh greg@"$region".$Domain: &> /dev/null
                chkresult
              done
            summary
            exit 0
   ...
   config ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                scp Sitespeed/config.json greg@"$region".$Domain:~/tld &> /dev/null
                scp Sitespeed/config.json greg@"$region".$Domain:~/comp &> /dev/null
                chkresult
              done
            summary
            exit 0
     ...
     cron ) if [ $# -ne 2 ]; then
            ...
            for region in $Regions
              do
                case $2 in
                  list ) echo -e "\nDisplaying "$region""
                         ssh greg@"$region".$Domain crontab -l
                         chkresult
                         ;;
                update ) echo -n "Starting "$region" ... "
                         sed 's/XXX/\'$region'/' Sitespeed/mycron.UPD > Sitespeed/mycron.REG
                         scp Sitespeed/mycron.REG greg@"$region".$Domain:mycron &> /dev/null
                         ssh greg@"$region".$Domain crontab mycron &> /dev/null
                         chkresult
                         ssh greg@"$region".$Domain rm mycron &> /dev/null
                         ;;
                delete ) echo -n "Starting "$region" ... "
                         ssh greg@"$region".$Domain crontab -r &> /dev/null
                         chkresult
                         ;;
                esac         
              done
   ...
   docker ) if [ $# -ne 2 ]; then
            ...
            for region in $Regions
              do
                echo -n "Starting "$region" ... "
                ssh greg@"$region".$Domain docker system prune --all --volumes -f &> /dev/null
                ssh greg@"$region".$Domain docker pull sitespeedio/sitespeed.io:$2 &> /dev/null
                chkresult
              done
            summary
            exit 0
     ...
     cwv ) if [[ $# -ne 2 && $# -ne 3 ]]; then
               echo -e "\ncwv requires an additional 1 or 2 arguments\n"
               exit 1
            fi
            echo "script seed" | tr ' ' '\n' | grep -F -x -q $2
            if [ $? -eq 1 ]; then
               echo -e "\narg2 must be script or seed\n"
               exit 1
            fi
            case $2 in
              script ) echo -n "Copying gpsi.sh to US-East... "
                       scp cwv.sh greg@US-East.$Domain: &> /dev/null
                       chkresult
                       summary
                       exit 0
                       ;;              
                seed ) if [ $# -ne 3 ]; then
                          echo -e "\nseed requires a filename\n"
                          exit 1
                       fi        
                       ...
                       echo -n "Copying $3.txt to tld on US-East... "
                       scp Seeds/$3.txt greg@US-East.$Domain:~/tld/ &> /dev/null
                       chkresult
                       summary
                       exit 0
                       ;;
    ...
    index ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                scp Portal/index.html greg@"$region".$Domain:~/portal &> /dev/null
                chkresult
              done
            summary
            exit 0
      ...
      log ) if [ $# -ne 2 ]; then
            ...
            for region in $Regions
              do
                case $2 in
                  check ) ssh greg@"$region".$Domain ls ./logs/tld.*.msg.log &> /dev/null
                          if [ $? -eq 0 ]; then
                             tldErrorCnt=$(ssh greg@"$region".$Domain grep ERROR ./logs/tld.*.msg.log | wc -l)
                            else
                              tldErrorCnt="n/a"
                          fi
                          ssh greg@"$region".$Domain ls ./logs/comp.*.msg.log &> /dev/null
                          if [ $? -eq 0 ]; then
                             compErrorCnt=$(ssh greg@"$region".$Domain grep ERROR ./logs/comp.*.msg.log | wc -l)
                            else
                              compErrorCnt="n/a"
                          fi
                          ...
                          ;;
                 delete ) echo -n "Starting "$region" ... "
                          ssh greg@"$region".$Domain rm ./logs/*.log
                          chkresult
                          ;;
                esac         
              done
   ...
   master ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                scp Sitespeed/master.sh greg@"$region".$Domain: &> /dev/null
                chkresult
              done
            summary
            exit 0
    ...
    nginx ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                ssh greg@"$region".$Domain sudo ./nginx.sh &> /dev/null
                chkresult
              done
            summary
            exit 0
    ...
    reset ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                ssh -q greg@"$region".$Domain ./reset.sh 2>/dev/null
                chkresult
              done
            summary
            exit 0
     ...
     seed ) if [ $# -ne 3 ]; then
            ...
            for region in $Regions
              do
                echo -n "Starting "$region" ... "
                scp Seeds/$3.txt greg@"$region".$Domain:~/$2 &> /dev/null
                chkresult
              done
            summary
            exit 0
      ...
      sro ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                scp Portal/sureroute-test-object.html greg@"$region".$Domain:~/portal &> /dev/null
                chkresult
              done
            summary
            exit 0
   ...
   update ) for region in $Regions
              do
                echo -n "Starting "$region" ... "
                ssh greg@"$region".$Domain sudo yum -y update &> /dev/null
                chkresult
              done
            summary
            exit 0
