...
Green='\033[0;32m'
NoColor='\033[0m'
Options="all update docker portal seed reset nginx cron logs custom cert graphite grafana storage"
...
if [[ "$1" == "--help" || "$1" == "-h" || "$1" == "/?" || $# -eq 0 ]]; then
   ...
   echo -e "\t${Green}graphite${NoColor}   Update Grafana\n"
...
## This is an undocumented argument. The purpose is to perform any ad-hoc commands
   custom ) Regions="PSI-CrUX"
            for region in $Regions
              do
                echo "Starting "$region" ... "
                scp -r Portal/* "$region".$Domain:~/portal/ &> /dev/null
                scp Sitespeed/nginx.sh "$region".$Domain: &> /dev/null
              done
            exit 0
            ;;
esac