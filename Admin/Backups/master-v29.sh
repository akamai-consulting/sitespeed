...
echo "sitespeed_usage.$graphdir.$2.$3.duration $runtime `date +%s`" | nc graphite 2003

# Log local image usage to Graphite
echo "sitespeed_usage.$graphdir.$2.$3.images `du -s $(pwd)/portal/images/$2 | awk '{print $1}'` `date +%s`" | nc graphite 2003

# Log disk usage for test type to Graphite
echo "sitespeed_usage.$graphdir.disk `ssh graphite du -s graphite-storage/whisper/sitespeed_io/$graphdir | awk '{print $1}'` `date +%s`" | nc graphite 2003

# Log current error count in test type to Graphite
ErrorCnt=$(grep -i -E 'error|unhandled|fail|caught|invalid|undefined|timeout' ./logs/$1.msg.log | wc -l)
echo "sitespeed_usage.$graphdir.errors $ErrorCnt `date +%s`" | nc graphite 2003

# Log disk usage for regional location to Graphite
echo "sitespeed_usage.$graphdir.$2.$3.disk `ssh graphite du -s graphite-storage/whisper/sitespeed_io/$graphdir/$2/$3 | awk '{print $1}'` `date +%s`" | nc graphite 2003

# Log disk usage for test name to Graphite
echo "sitespeed_usage.$graphdir.$2.disk `ssh graphite du -s graphite-storage/whisper/sitespeed_io/$graphdir/$2 | awk '{print $1}'` `date +%s`" | nc graphite 2003
...
case $1 in
   tld ) find $(pwd)/$1/sitespeed-result/$2/ -mmin +120 | xargs rm -rf
         ;;

  comp ) find $(pwd)/$1/sitespeed-result/$2/ -mmin +120 | xargs rm -rf 
         ;;
esac
